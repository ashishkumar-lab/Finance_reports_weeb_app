...
export default async function B2BClientPerformancePage() {
  ...
  const hasAccess = session.user.isAdmin || (
    session.user.dbUserId
      ? (await getUserPermissions(session.user.dbUserId)).includes("dash-b2b-client-performance")
      : false
  );
  ...
  return (
    <AppLayout userName={session.user?.name ?? "User"} isAdmin={session.user.isAdmin}>
      {!hasAccess ? (
        <DashboardLocked dashboardId="dash-b2b-client-performance" dashboardName="B2B Client Performance" />
      ) : (
        <Suspense fallback={
