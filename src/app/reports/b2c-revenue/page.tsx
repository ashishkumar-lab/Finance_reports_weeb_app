...
export default async function B2CRevenuePage() {
  ...
  const hasAccess = session.user.isAdmin || (
    session.user.dbUserId
      ? (await getUserPermissions(session.user.dbUserId)).includes("b2c-revenue")
      : false
  );
  ...
  return (
    <AppLayout userName={session.user?.name ?? "User"} isAdmin={session.user.isAdmin}>
      {!hasAccess ? (
        <DashboardLocked dashboardId="b2c-revenue" dashboardName="B2C Revenue Report" />
      ) : (
        <ReportTableClient reportId="b2c-revenue" reportName="B2C Revenue Report" columns={COLUMNS} />
      )}
    </AppLayout>
  );
}
