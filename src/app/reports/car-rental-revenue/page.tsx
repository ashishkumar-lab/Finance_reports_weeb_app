...
export default async function CarRentalRevenuePage() {
  ...
  const hasAccess = session.user.isAdmin || (
    session.user.dbUserId
      ? (await getUserPermissions(session.user.dbUserId)).includes("car-rental-revenue")
      : false
  );
  ...
  return (
    <AppLayout userName={session.user?.name ?? "User"} isAdmin={session.user.isAdmin}>
      {!hasAccess ? (
        <DashboardLocked dashboardId="car-rental-revenue" dashboardName="Car Rental Revenue Report" />
      ) : (
        <ReportTableClient reportId="car-rental-revenue" reportName="Car Rental Revenue Report" columns={COLUMNS} />
      )}
    </AppLayout>
  );
}
