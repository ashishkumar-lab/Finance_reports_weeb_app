...
WHERE b.status_id IN (4, 8)
  AND t.trip_end_time BETWEEN ? AND ?
ORDER BY t.trip_end_time ASC
`;
...
export async function GET(req: NextRequest) {
  ...
  if (!session.user.isAdmin) {
    ...
    const allowed = await hasReportPermission(session.user.dbUserId, "car-rental-revenue");
  ...
  try {
    ...
    const buffer = await generateExcel("Car Rental Revenue", COLUMNS, rows);
    await logDownload("Car Rental Revenue Report", startDate, endDate, session.user?.email ?? "unknown");

    return new NextResponse(new Uint8Array(buffer), {
      status: 200,
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="car_rental_revenue_${startDate}_${endDate}.xlsx"`,
      },
    });
  } catch (err) {
    console.error("[Car Rental Revenue] Error:", err);
    await logDownload("Car Rental Revenue Report", startDate!, endDate!, session.user?.email ?? "unknown", "error");
