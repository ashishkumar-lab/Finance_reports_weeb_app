...
export async function GET(req: NextRequest) {
  ...
  if (!session.user.isAdmin) {
    ...
    try {
      const allowed = await hasReportPermission(session.user.dbUserId, "coach-revenue");
  ...
  try {
    const rows = await driveuQueryLong<Record<string, unknown>>(COACH_REVENUE_QUERY, [startDate, endDate]);
    ...
    const buffer = await generateExcel("Coach Revenue", COLUMNS, rows);
    await logDownload("Coach Revenue Report", startDate, endDate, session.user?.email ?? "unknown");

    return new NextResponse(new Uint8Array(buffer), {
      status: 200,
      headers: {
        "Content-Type": "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
        "Content-Disposition": `attachment; filename="coach_revenue_${startDate}_${endDate}.xlsx"`,
      },
    });
  } catch (err) {
    console.error("[Coach Revenue] Error:", err);
    await logDownload("Coach Revenue Report", startDate!, endDate!, session.user?.email ?? "unknown", "error");
